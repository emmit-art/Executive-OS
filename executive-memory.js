...
async function rememberObject(o){
  ...
  const org=areaName(o.object_data?.area||null);
  if(workspace_id){
    const ws=await sb.from("workspaces").select("area").eq("id",workspace_id).maybeSingle();
    if(ws.data?.area){const id=await upsertEntity(owner_id,"organization",areaName(ws.data.area),{workspace_id,confidence:.98});await linkEntity(owner_id,"executive_object",source_id,id,"belongs_to",.98)}
  }
...
async function syncMemory(){
  ...
  const{data,error}=await sb.from("executive_objects").select("*").eq("owner_id",session.user.id).order("created_at",{ascending:false}).limit(100);
...
function installUI(){
  ...
  if(desktop){const b=document.createElement("button");b.dataset.tab="memory";b.textContent="Memory";desktop.appendChild(b)}
  const more=document.querySelector("#more .more-grid");
  if(more){const b=document.createElement("button");b.className="more-link";b.dataset.tab="memory";b.innerHTML="<span>⌁</span><div><strong>Executive Memory</strong><small>People, projects, and history</small></div>";more.prepend(b)}
  const main=document.querySelector("main");
  const section=document.createElement("section");section.id="memory";section.className="section";section.innerHTML=`<div class="page-title"><div class="eyebrow">Executive Memory</div><h2>Everything connected</h2></div><div class="grid"><div class="card s12"><div class="memory-search-row"><input id="memorySearch" class="field" placeholder="Search a person, company, project, or topic"><button id="memorySearchBtn" class="btn">Search</button></div><div id="memorySummary" class="status">Search your connected memory.</div></div><div class="card s5"><div class="eyebrow">Known entities</div><div id="memoryEntities"></div></div><div class="card s7"><div class="eyebrow">Memory briefing</div><div id="memoryResults"></div></div></div>`;main.appendChild(section);
  document.querySelectorAll('[data-tab="memory"]').forEach(b=>b.addEventListener("click",()=>switchMemory()));
  document.getElementById("memorySearchBtn").onclick=searchMemory;document.getElementById("memorySearch").addEventListener("keydown",e=>{if(e.key==="Enter")searchMemory()});
  const style=document.createElement("style");style.textContent=`.memory-search-row{display:flex;gap:10px}.memory-search-row .field{flex:1}.memory-entity{padding:12px 0;border-bottom:1px solid rgba(130,160,200,.12)}.memory-entity:last-child{border-bottom:0}.memory-entity small{display:block;color:var(--muted,#9eb1cc);margin-top:3px}.memory-result{padding:14px;border:1px solid rgba(125,170,230,.18);border-radius:14px;margin:10px 0}.memory-result h3{margin:0 0 8px}.memory-tags{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px}.memory-tags span{font-size:.72rem;padding:5px 8px;border-radius:999px;background:rgba(80,130,210,.12);color:#b9d4ff}@media(max-width:600px){.memory-search-row{flex-direction:column}}`;document.head.appendChild(style);
}
...
async function searchMemory(){
  await loadMemory();const q=norm(document.getElementById("memorySearch")?.value);const out=document.getElementById("memoryResults");if(!q){out.innerHTML="<div class='empty'>Enter a name, project, company, or topic.</div>";return}
  const matched=entities.filter(e=>e.normalized_name.includes(q)||q.includes(e.normalized_name));
  if(!matched.length){out.innerHTML=`<div class="empty">No connected memory found for “${esc(q)}” yet.</div>`;return}
  const cards=matched.map(e=>{
    ...
    const related=objects.filter(o=>ids.has(o.id));const open=related.filter(o=>o.status!=="completed"&&o.status!=="cancelled");
    const last=related[0];const taskMatches=tasks.filter(t=>norm(t.related_person)===e.normalized_name||norm(t.title).includes(e.normalized_name));
    const waitMatches=waiting.filter(w=>norm(w.person_or_company)===e.normalized_name||norm(w.item).includes(e.normalized_name));
    const next=taskMatches.find(t=>!['completed','cancelled'].includes(t.status));
    return `<div class="memory-result"><h3>${esc(e.name)}</h3><p>${esc(e.entity_type)}${e.role?` · ${esc(e.role)}`:""}${e.relationship?` · ${esc(e.relationship)}`:""}</p><p><b>${open.length}</b> open connected item${open.length===1?"":"s"} · <b>${waitMatches.length}</b> waiting item${waitMatches.length===1?"":"s"}</p>${last?`<p><b>Latest:</b> ${esc(last.title)}${last.reason?` — ${esc(last.reason)}`:""}</p>`:""}${next?`<p><b>Recommended next action:</b> ${esc(next.title)}</p>`:""}<div class="memory-tags">${related.slice(0,5).map(o=>`<span>${esc(o.object_type)}: ${esc(o.title)}</span>`).join("")}</div></div>`
