(()=>{
  ...
  function render(a,host,showProposal=false){
    ...
    if(a.status!=='pending'){
      card.append(node('p',a.error_message || (a.status==='declined'?'Declined. Nothing was executed.':a.execution_status==='succeeded'?'Diagnostic completed once. No external action was performed.':`Execution: ${a.execution_status}`)));
